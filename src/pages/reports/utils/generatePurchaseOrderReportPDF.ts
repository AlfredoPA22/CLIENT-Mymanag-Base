...
export const generatePurchaseOrderReportPDF = (
  ...
  doc.text("REPORTE DE COMPRAS", MARGIN, 16);
  ...
  doc.setTextColor(...INK);
  doc.text(
    `Proveedor: ${filters.provider || "Todos"}     Estado: ${filters.status || "Todos"}`,
    MARGIN + 18,
    filterY
  );
  ...
  // ── TABLE ────────────────────────────────────────────────
  autoTable(doc, {
    head: [["Código", "Fecha", "Proveedor", "Estado", `Total (${currency})`]],
    headStyles: {
  ...
  doc.setFontSize(6.5);
  doc.setTextColor(...INK_LIGHT);
  doc.text("Inventasys — Reporte de Compras", MARGIN, 287);
